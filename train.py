...
class FLAGS(NamedTuple):
    ...
    LOG_ROOT = os.path.join('workspace', 'runs')
    RUN_NAME = "model"
    MAX_EPOCH = 200
    INIT_LR = 1e-1
    N_WORKERS = 0
...
def forward(images, is_training: bool):
    ...
    net = resnet_cifar.ResNet20(num_classes=10, bn_config={'decay_rate': 0.9})
...
#train_dataset = datasets.CIFAR10(FLAGS.DATA_ROOT, train=True, download=True)
#test_dataset = datasets.CIFAR10(FLAGS.DATA_ROOT, train=False)
...
if __name__ == '__main__':
    ...
    def save_pickle(name=None):
        ...
        pickle_path = os.path.join(FLAGS.LOG_ROOT, f'{name}.pickle')
    ...
    def load_pickle(name=None):
        ...
        try:
          ...
          return TrainState(state_dict['params'], state_dict['state'], state_dict['opt_state'])
        except OSError:
          pass

    if (it := load_pickle()) is not None:
      train_state = it
    else:
      breakpoint()
    ...
    for epoch in range(FLAGS.MAX_EPOCH):
      with tqdm(range(len(train_dataset)), bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}', smoothing=1.) as pbar:
